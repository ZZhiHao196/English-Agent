// import { getCurrentSettings } from '../settings/settings.js'; // Remove old import
...
export const getConfig = (options = {}) => { // Add options parameter
    ...
    if (forceGemini || isTtsAgent || isCallPage) {
        ...
        if (forceGemini) {
            ...
            const isTtsSupportedModel = modelToUse.includes('-exp') || modelToUse === 'gemini-2.0-flash-live-001';
            if (!isTtsSupportedModel) {
                console.warn(`[Config] Selected model ${modelToUse} does not support WebSocket bidirectional calls. Using gemini-2.0-flash-exp instead.`);
                modelToUse = 'gemini-2.0-flash-exp'; // 强制使用支持的模型
            }
        }
    ...
    // Base configuration using determined model
    const config = {
        model: modelPath,
        generation_config: {
            // Use Gemini settings if forced, otherwise use current settings
        ...
        system_instruction: {
            parts: [{
                // System instructions are usually generic enough, but keep page logic
                text: settings.systemInstructions || 
                    (isCallPage ? 
        ...
        config.generation_config.speech_config = {
            voice_config: {
                prebuilt_voice_config: {
                    // Voice type is relevant for Gemini speech
                    voice_name: settings.voiceType || DEFAULT_SETTINGS.voiceType
                }
            }
        };
        console.debug("[Config] Added speech_config:", config.generation_config.speech_config);
    }
